...
fn main() {
    ...
    if run_all || (filter.as_deref() != Some("52") && filter.as_deref() != Some("53")) {
        ...
        let worldgen =
            ProceduralWorldGenerator::new(WorldGenConfig::new(WorldSeed::from_u64(1337)));
        ...
        let compressed_bytes = {
            let start = Instant::now();
            let compressed = serialize_and_compress_chunk(&terrain_chunk, &registry)
                .expect("Kompresi chunk gagal");
            let elapsed = start.elapsed();
            println!(
        ...
        // 11. Benchmark Connectivity BFS
        {
            ...
            while let Some(current) = queue.pop_front() {
                ...
                for offset in &neighbors {
                    ...
                    {
                        let block = terrain_chunk.get_voxel(
                            next.x as usize,
                            next.y as usize,
                            next.z as usize,
                        );
        ...
        // 20. Benchmark Event-Driven Localized Structural Connectivity (Gate A)
        {
            ...
            use omnisia::structure::connectivity::{
                check_structural_connectivity, ConnectivityConfig,
            };
            ...
            let stone_id = registry
                .resolve_material_id(
                    &omnisia::modding::resource_id::ResourceId::core("stone").unwrap(),
                )
                .unwrap();
        ...
        // ========================================================================
        {
            ...
            for i in 0..count {
                ...
                let aabb = Aabb::try_new(Vec3::new(x, y, z), Vec3::new(x + 2.0, y + 2.0, z + 2.0))
                    .unwrap();
                let btype = if i % 5 == 0 {
                    BodyType::Static
                } else {
        ...
        // ========================================================================
        {
            ...
            for i in 0..count {
                ...
                let aabb =
                    Aabb::try_new(Vec3::new(x, y, z), Vec3::new(x + 12.0, y + 12.0, z + 12.0))
                        .unwrap();
                broadphase
                    .insert(BroadphaseProxy::new(
                        RigidBodyId(i as u64 + 1),
        ...
        // ========================================================================
        {
            use glam::Quat;
            use omnisia::physics::{
                collide, BoxShape, Capsule, Collider, ColliderId, RigidBodyId, Shape, Sphere,
                Transform,
            };
        ...
        // ========================================================================
        {
            ...
            for &(cfg_name, restitution, friction) in &configurations {
                for &num_contacts in &contact_counts {
                    ...
                    let body_b =
                        RigidBody::new_static(body_b_id, Vec3::ZERO, Quat::IDENTITY).unwrap();
        ...
        // ========================================================================
        {
            use glam::{Mat3, Quat, Vec3};
            use omnisia::physics::{
                BoxShape, Collider, ColliderId, PhysicsWorld, PhysicsWorldConfig,
                PlayerBridgeConfig, PlayerRigidBodyBridge, RigidBody, RigidBodyId, Shape,
                Transform,
            };
            use omnisia::player::PlayerController;
        ...
        // 49. Benchmark RigidBody Stress & Scaling (Phase 9.12)
        {
            ...
            {
                use omnisia::impact::{
                    AffectedVolume, DeterministicImpactPipeline, ImpactEvent, ImpactId,
                    ImpactSource,
                };
            ...
            // ========================================================================
            {
                ...
                for &r in &[2.0f32, 4.0, 8.0, 16.0] {
                    ...
                    let mut last_len = 0;
                    for _ in 0..iterations {
                        let tx = CraterGenerator::generate(center, r, &policy, &materials, &store)
                            .unwrap();
                        last_len = tx.len();
                    }
                    let dur = start.elapsed();
                ...
                let tx =
                    CraterGenerator::generate(center, 4.0, &policy, &materials, &store).unwrap();
    ...
    if run_all || filter.as_deref() == Some("52") {
        ...
        use omnisia::physics::aggregate::{
            calculate_aggregate_mass_properties, generate_aggregate_colliders,
            AggregateColliderStrategy, OrientationQuantizationPolicy,
        };
        use omnisia::physics::world::{PhysicsWorld, PhysicsWorldConfig};
        use omnisia::streaming::store::ChunkStore;
        use omnisia::structure::aggregate::DetachedAggregate;
        ...
        println!("------------------------------------------------------------");
        println!("[BENCHMARK 52] Impact -> Structure -> Physics Integration (Phase 10.3)");
        println!("------------------------------------------------------------");

        let mat_anchor = MaterialId(1);
        let mat_wood = MaterialId(2);

        let make_world_store = |min_c: IVec3, max_c: IVec3| -> ChunkStore {
            let mut store = ChunkStore::new();
        ...
        let make_structural_sys = || -> StructuralSystem {
            let mut policy = AnchorPolicy::default();
            policy.register_anchor_material(mat_anchor);
            StructuralSystem::new(policy)
        };

        // ----------------------------------------------------------------
        // Profile 1: Single Tiny Aggregate (1 voxel)
        // ----------------------------------------------------------------
        {
            let iters = 200;
            let start = Instant::now();
            for _ in 0..iters {
                let mut store = make_world_store(IVec3::splat(-1), IVec3::splat(1));
                let mut struct_sys = make_structural_sys();
                ...
                let impact = ImpactEvent::builder(ImpactId(1), Vec3::new(0.25, 0.75, 0.25), 0.5)
                    .impulse(10.0)
                    .direction(Vec3::Y)
                    .build()
                    .unwrap();
            ...
            let dur = start.elapsed();
            let us_per_op = dur.as_micros() as f64 / iters as f64;
            println!(
                "  [Profile 1] Single Tiny Aggregate (1 voxel): {:.2} µs/op (iters: {})",
                us_per_op, iters
            );
        }

        // ----------------------------------------------------------------
        // Profile 2: Medium Aggregate (10 voxels)
        // ----------------------------------------------------------------
        {
            let iters = 200;
            let start = Instant::now();
            for _ in 0..iters {
                ...
                store.set_voxel_world(IVec3::new(0, 1, 0), VoxelBlock::new(mat_wood));
                for y in 2..=11 {
                    store.set_voxel_world(IVec3::new(0, y, 0), VoxelBlock::new(mat_wood));
                }
                ...
                let impact = ImpactEvent::builder(ImpactId(2), Vec3::new(0.25, 0.75, 0.25), 0.5)
                    .impulse(20.0)
                    .direction(Vec3::X)
                    .build()
                    .unwrap();
            ...
            let us_per_op = dur.as_micros() as f64 / iters as f64;
            println!(
                "  [Profile 2] Medium Aggregate (10 voxels): {:.2} µs/op (iters: {})",
                us_per_op, iters
            );
        }

        // ----------------------------------------------------------------
        // Profile 3: Large Aggregate (100 voxels)
        // ----------------------------------------------------------------
        {
            let iters = 100;
            let start = Instant::now();
            for _ in 0..iters {
                ...
                for x in 0..10 {
                    for z in 0..10 {
                        store.set_voxel_world(IVec3::new(x, 2, z), VoxelBlock::new(mat_wood));
                    }
                }
            ...
            let us_per_op = dur.as_micros() as f64 / iters as f64;
            println!(
                "  [Profile 3] Large Aggregate (100 voxels): {:.2} µs/op (iters: {})",
                us_per_op, iters
            );
        }

        // ----------------------------------------------------------------
        // Profile 4: Very Large Aggregate (1,000 voxels)
        // ----------------------------------------------------------------
        {
            let iters = 20;
            let start = Instant::now();
            for _ in 0..iters {
                ...
                for y in 2..12 {
                    for x in 0..10 {
                        for z in 0..10 {
                            store.set_voxel_world(IVec3::new(x, y, z), VoxelBlock::new(mat_wood));
                        }
                    }
                }
            ...
            let us_per_op = dur.as_micros() as f64 / iters as f64;
            println!(
                "  [Profile 4] Very Large Aggregate (1,000 voxels): {:.2} µs/op (iters: {})",
                us_per_op, iters
            );
        }

        // ----------------------------------------------------------------
        // Profile 5: Multi-Chunk Aggregate (Crossing chunk boundaries)
        // ----------------------------------------------------------------
        {
            let iters = 100;
            let start = Instant::now();
            for _ in 0..iters {
                let mut store = make_world_store(IVec3::new(-1, -1, -1), IVec3::new(2, 1, 1));
                let mut struct_sys = make_structural_sys();
                let mut phys_world = PhysicsWorld::new(PhysicsWorldConfig::default());

                store.set_voxel_world(IVec3::new(24, 5, 5), VoxelBlock::new(mat_anchor));
                store.set_voxel_world(IVec3::new(25, 5, 5), VoxelBlock::new(mat_wood));
                // 64-voxel beam spanning across boundary x=31/32 (from x=26 to x=41, 2x2 in yz)
                for x in 26..=41 {
                    for y in 5..=6 {
                        for z in 5..=6 {
                            store.set_voxel_world(IVec3::new(x, y, z), VoxelBlock::new(mat_wood));
                ...
                let mut tx = VoxelEditTransaction::new();
                tx.add_edit(VoxelEdit::remove(IVec3::new(25, 5, 5)));
                let commit_res = tx.commit(&mut store).unwrap();

                let impact = ImpactEvent::builder(ImpactId(5), Vec3::new(12.75, 2.75, 2.75), 0.5)
                    .impulse(30.0)
                    .direction(Vec3::X)
                    .build()
                    .unwrap();
            ...
            let us_per_op = dur.as_micros() as f64 / iters as f64;
            println!(
                "  [Profile 5] Multi-Chunk Aggregate (64 voxels, cross-boundary): {:.2} µs/op (iters: {})",
                us_per_op, iters
            );
        }

        // ----------------------------------------------------------------
        // Profile 6: Multiple Detached Aggregates (Split from single impact)
        // ----------------------------------------------------------------
        {
            let iters = 100;
            let start = Instant::now();
            for _ in 0..iters {
                ...
                // Central anchor at (16, 16, 16)
                store.set_voxel_world(IVec3::new(16, 16, 16), VoxelBlock::new(mat_anchor));
                // 4 arms extending: +X, -X, +Z, -Z (each 4 voxels = 16 voxels total, 4 bodies)
                for i in 1..=4 {
                    store.set_voxel_world(IVec3::new(16 + i, 16, 16), VoxelBlock::new(mat_wood));
                    store.set_voxel_world(IVec3::new(16 - i, 16, 16), VoxelBlock::new(mat_wood));
                    store.set_voxel_world(IVec3::new(16, 16, 16 + i), VoxelBlock::new(mat_wood));
                    store.set_voxel_world(IVec3::new(16, 16, 16 - i), VoxelBlock::new(mat_wood));
                }

                let mut tx = VoxelEditTransaction::new();
                tx.add_edit(VoxelEdit::remove(IVec3::new(16, 16, 16)));
                let commit_res = tx.commit(&mut store).unwrap();

                let impact = ImpactEvent::builder(ImpactId(6), Vec3::new(8.25, 8.25, 8.25), 0.5)
                    .impulse(40.0)
                    .direction(Vec3::Y)
                    .build()
                    .unwrap();
            ...
            let us_per_op = dur.as_micros() as f64 / iters as f64;
            println!(
                "  [Profile 6] Multiple Detached Aggregates (4 bodies, 16 voxels): {:.2} µs/op (iters: {})",
                us_per_op, iters
            );
        }

        // ----------------------------------------------------------------
        // Profile 7: Structural Reconciliation Latency Profile
        // ----------------------------------------------------------------
        {
            let iters = 500;
            ...
            for _ in 0..iters {
                ...
                let start = Instant::now();
                let aggregates = struct_sys.reconcile_events(&events, &mut store);
                total_ns += start.elapsed().as_nanos();
                std::hint::black_box(aggregates);
            }
            let us_per_reconcile = (total_ns as f64 / iters as f64) / 1_000.0;
            println!(
                "  [Profile 7] Structural Reconciliation Latency: {:.2} µs/call (10 voxels, iters: {})",
                us_per_reconcile, iters
            );
        }

        // ----------------------------------------------------------------
        // Profile 8: Physicalization Latency Profile (Breakdown: Mass/Inertia + Colliders + Body Registration)
        // ----------------------------------------------------------------
        {
            let iters = 500;
            let mut voxels = Vec::new();
            for y in 0..10 {
                for x in 0..10 {
                    voxels.push((IVec3::new(x, y, 0), VoxelBlock::new(mat_wood)));
                }
            }
            let agg = DetachedAggregate::from_world_voxels(100, &voxels).unwrap();
            ...
            let mass_us = (start.elapsed().as_nanos() as f64 / iters as f64) / 1_000.0;

            // 2. Colliders
            let props = calculate_aggregate_mass_properties(&agg, None).unwrap();
            let start = Instant::now();
            for _ in 0..iters {
                let mut next_col = 1u64;
                let cols = generate_aggregate_colliders(
                    omnisia::physics::RigidBodyId(1),
                    &agg,
            ...
            let col_us = (start.elapsed().as_nanos() as f64 / iters as f64) / 1_000.0;

            // 3. Full physicalization into world
            let start = Instant::now();
            for _ in 0..iters {
                let mut world = PhysicsWorld::new(PhysicsWorldConfig::default());
                let id = world
            ...
            let full_us = (start.elapsed().as_nanos() as f64 / iters as f64) / 1_000.0;

            println!("  [Profile 8] Physicalization Breakdown (100 voxels):");
            println!("    - Mass & Inertia: {:.2} µs", mass_us);
            println!("    - Collider Gen:   {:.2} µs", col_us);
            println!("    - Total Full Phys:{:.2} µs (iters: {})", full_us, iters);
        }

        // ----------------------------------------------------------------
        // Profile 9: Reintegration Latency Profile (Prepare + Validate + Commit)
        // ----------------------------------------------------------------
        {
            let iters = 200;
            let mut voxels = Vec::new();
            for y in 0..10 {
                for x in 0..10 {
                    voxels.push((IVec3::new(x + 5, y + 5, 5), VoxelBlock::new(mat_wood)));
                }
            }
            let agg = DetachedAggregate::from_world_voxels(200, &voxels).unwrap();

            let mut total_reint_ns = 0u128;
            for _ in 0..iters {
                let mut store = make_world_store(IVec3::splat(-1), IVec3::splat(1));
                let mut world = PhysicsWorld::new(PhysicsWorldConfig::default());
                let dyn_id = world
                ...
                world
                    ...
                    .unwrap();
                total_reint_ns += start.elapsed().as_nanos();
            }
            let us_per_reint = (total_reint_ns as f64 / iters as f64) / 1_000.0;
            println!(
